...
glmnet_cv <- cv.glmnet(x = as.matrix(x_train),y = y_train, alpha=1)
ridge_fit <- glmnet(x = as.matrix(x_train),y = y_train, alpha=1, lambda = glmnet_cv$lambda.1se)
...
write.csv(y_pred, file="ridge_test_fm2.csv", row.names = FALSE)
write.csv(y_pred_train, file="ridge_train_fm2.csv", row.names = FALSE)
